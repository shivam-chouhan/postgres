...
router.post('/savedata',(req , res)=>{
  let data = req.body;
  pool.query(`INSERT INTO users(first_name , middle_name ,last_name, email, phone, role ,address )values('${data.first_name}','${data.middle_name}','${data.last_name}','${data.email}','${data.phone}','${data.role}','${data.address}')`)
  res.send('success');
  res.status(200)
})
